# Changelog（アーカイブ: リパーパス前 / 旧 WSL-kernel-watcher）

このファイルは、本プロジェクトが「Squirrel Notifier」へリパーパスされる前
（リポジトリ名 `WSL-kernel-watcher` 時代）の変更履歴を保全したものです。
現行（v0.1.0 以降）の履歴は [`../CHANGELOG.md`](../CHANGELOG.md) を参照してください。
