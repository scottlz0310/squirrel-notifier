...
- [ ] **PR #198 のクラッシュ修正**（最優先。現状 main はレビュー通知を受けた時点で必ず落ちる）
- [ ] **PR #197 のログインダイアログ自動クローズ修正**（失敗理由がユーザーに届かない）
- [ ] 修正後に 198-4 〜 198-9 を再検証
- [ ] device flow の成功パス（成功ダイアログ・InfoBar クローズ・購読再開）を、ユーザー本人の承認を伴って確認
- [ ] preflight 失敗時の `LastError` 未設定を修正
- [ ] `CheckSubscriberVersionAsync` の stderr 未読を修正
