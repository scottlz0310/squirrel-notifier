...
サイクル全体の運用手順は [docs/review-cycle-automation.md](docs/review-cycle-automation.md) にまとめています。

別のレビューの実行中、起動処理中（Auto-Pause の強行確認ダイアログの表示中など）、または Auto-Pause が Paused のときに届いたイベントは保留し、実行が終わった時点、起動処理がレビューを起動せずに終わった時点、または Auto-Pause の解除を確認した時点で、受信順に 1 件ずつ自動起動を再評価します。同じ PR のイベントは 1 件にまとめ、新しいほうの reason で起動します。再評価でも自動起動の設定・Auto-Pause・PR の状態（マージ・クローズ済みなら起動しない）を確認し、イベント一覧から削除されたイベントは起動しません。保留と再評価の結果は「Recent activity」に記録し、保留したことはトレイ通知にも「レビューを保留中（理由）」として表示します。前の実行のライブログウィンドウが開いている間に起動したレビューは、そのウィンドウが閉じた後にウィンドウを表示します。保留はアプリの再起動で失われますが、イベントは一覧に残るため手動で起動できます。
...
（GitHub-hosted の headless E2E）で検証します。実デスクトップの確認は、リリース前に
AI エージェントがランブックに沿って行う方針です（#433）。
