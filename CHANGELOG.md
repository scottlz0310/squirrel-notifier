...
### Added
- Recent review events の巡回で PR がマージ済み・クローズ済みと判明したら、その PR の reviewer 作業領域（`launcher-workspace\reviewer\<owner>\<repo>\<PR 番号>`。clone と一時ファイルを含む）を削除するようにした。削除するのはアプリが PR 単位で作ったディレクトリだけで、同じ PR の reviewer が実行中なら見送り、reviewer の終了時に再試行する（ファイルのロックなどで失敗した場合も同じ）。読み取り専用の git object も削除する。領域内のリンク（symlink / junction）と、作業領域そのものがリンクの場合は、先をたどらずリンク自体だけを消す。owner / repo の階層がリンクの場合は削除しない。Recent review events から外れた PR の片付け（TTL）と手動のクリーンナップは後続（#403）
...
- Codex の reviewer / reviewed launcher が skill を `/` で呼び出しており、Codex が skill 呼び出しとして認識しなかったのを修正した。Codex の構文に合わせて `$thread-owl-pr-reviewer` / `$review-raven-thread-owl-cycle` で呼び出し、未変更の旧既定値だけを `CodexSkillPromptPrefixMigrated` で移行する。自由編集された設定は変更しない。「コマンドをコピー」の出力も、`$` / `` ` `` を含む引数は PowerShell で変数展開されない単一引用符形式にした（#395）
...
- EC2 desktop runner を GitHub OIDC で起動し、runner online 待機後に E2E を実行し、workflow が起動した instance だけを cleanup で停止する運用を追加した（#361）
...
- reviewer / reviewed launcher の既定プロンプトを、MCP ツールの説明全文ではなく `thread-owl-pr-reviewer` / `review-raven-thread-owl-cycle` skill の明示呼び出しへ更新した（#353）。既存の未変更設定だけを一回限り移行し、skill の未配布・未対応 CLI は stderr と原因付き失敗メッセージへ残す
...
- Recent review events の行にある「レビューに対応」を、設定で表示を切り替えられるようにし、**既定を非表示**にした（#256）。レビュー指摘への対応は PR を実装した CLI エージェントが同じセッションで引き受けるほうが良く、このボタンは reviewed 側をコールドスタートする劣った導線になっていた。機能と reviewed launcher スロットの設定は退避手段として残し、設定で表示に切り替えれば従来どおり起動・コマンドコピーができる。切り替えは再起動なしで反映される
...
- 保留したイベントのトレイ通知に、保留した理由（Auto-Pause 中・別のレビューが実行中）を表示するようにした（#340）。これまでは自動起動しなかっただけのイベントと同じ文言だったため、無人運用で見送りに気づけなかった
- 「レビュー自動開始」で、別のレビューの実行中または起動処理中のために自動起動を見送ったイベントを保留し、実行終了後（起動処理がレビューを起動せずに終わった場合はその時点）に受信順で 1 件ずつ自動起動を再評価するようにした（#339）。これまでは見送ったまま再試行せず、手動で「レビューする」を押す必要があった。同じ PR のイベントは 1 件にまとめて新しいほうの reason で起動し、再評価でも自動起動の設定・Auto-Pause・PR の状態を確認する。イベント一覧から削除されたイベントと、マージ・クローズ済みの PR は起動しない。保留・再評価の結果は Recent activity に記録する。保留はメモリ上にのみ持ち、アプリの再起動で失われる（イベントは一覧に残るため手動で起動できる）
- 前の実行のライブログウィンドウが開いている間に起動したレビューのウィンドウを、これまでは表示しなかったが、そのウィンドウが閉じた後に起動順で表示するようにした（#339）。セッションの出力はウィンドウを開くまで保持されるため失われない

## [0.10.1] - 2026-09-16

Codex の Auto-Pause 誤発動、GitHub API のレート制限消費、呼出元キャンセルの誤変換などを修正したパッチリリースです。
...
- Codex の Luna Reserve Weekly 枠（bucket `base_model_inference`）の枯渇だけで Auto-Pause が発動していた問題を修正した（#335）。Auto-Pause は通常枠（bucket `codex` の primary / secondary）の使用率だけを判断材料にし、判定は bucket ID と window slot の組み合わせで行う（表示名や `windowDurationMins` には依存しない）。予約枠は情報表示のみに使い、自動モデル切替・フォールバック・consume 系 API の呼び出しには使わない。あわせてレートリミットの表示を `Codex — 5時間制限（全モデル）` / `Codex — Weekly制限（全モデル）` / `Codex — Luna Reserve Weekly制限（Luna専用・Auto-Pause対象外）` のようにサービス名と用途が分かる形にし、エージェント表示名を `Claude Code` / `Antigravity (agy)` / `Codex` へ揃えた（`settings.json` に保存される ID は変更なし）。ライブログのゲージは判定対象の枠を既定選択する。メイン UI の「レートリミット状態」一覧とレートリミットのリマインダー通知にもサービス名を含める（`Codex — Luna Reserve Weekly制限（…）`。MCP の `ratelimit://` リソース由来の枠は URI が出所を示すため従来どおりラベルのみ）
...
- `RateLimitRefreshCoordinator` と `ReviewStartCoordinator` が、呼出元のキャンセルを取得エラーの警告・起動失敗に変換していた問題を修正した（#333）。`CancellationToken` がキャンセルされた場合は残りのエージェント・URI の取得を打ち切って `OperationCanceledException` を伝播させ、`ReviewStartCoordinator` は再入抑止を解除して次の起動を受け付ける。HTTP のタイムアウト等、呼出元がキャンセルしていない `OperationCanceledException` は従来どおり取得エラー・起動失敗として扱う。現在の呼出元（`MainWindow`）は `CancellationToken.None` を渡しているため表示上の挙動は変わらない
...
- エージェント実行ウィンドウの参照保持とクローズ時の状態解除を `AgentExecutionWindowCoordinator` へ抽出し、`MainWindow.xaml.cs` にはウィンドウ生成と表示の UI 配線だけを残した（#262、#286）
