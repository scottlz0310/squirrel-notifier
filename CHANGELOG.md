...
### Added
- 購読開始の完了を待てる `McpSubscriptionService.StartAsync` を追加した（#208、epic #185）。従来の `Start()` は `void` で購読ループを投げて即座に戻るため、呼び出し側は購読が `Running` に到達したか `Error` で終わったかを知る手段がなく、「購読を開始してからレビューを登録する」導線（#209）を UI 側で実装できなかった。新 API は状態遷移を `StateChanged` だけで観測し（ポーリングしない）、`Started` / `Failed` / `TimedOut` / `Cancelled` を区別した `SubscriptionStartResult` を返す。既に `Running` なら即座に成功を返し、`Starting` 中の多重呼び出しは進行中の開始結果を待って購読プロセスを二重起動しない。停止処理中（`Stopping`）の呼び出しは、新しいループを起動した直後に `StopAsync` の後始末が `State = Stopped` を書き込んで状態と実体が食い違うのを避けるため、開始せずに失敗を返す。この判定は開始可否の確認と `State` / `_stopCts` / `_loopTask` の差し替えを同一の排他区間で行うことで実現しており、`StopAsync` の `Stopping` 遷移も同じロック配下に置いて開始・停止を直列化している（確認と生成が分かれていると、その隙間に停止が割り込んだ場合に停止中の購読ループを起動してしまい、`StopAsync` が新しいループを待ち続ける）。タイムアウトは DI 可能（既定 30 秒）。既存の `Start()` 呼び出し経路（起動時の自動開始・Start / Retry ボタン・トレイメニュー）は変更していない。なお mcp-gateway の認証要求はこの結果では表現しない。購読開始の可否は preflight（subscriber の `--help`）だけで決まり、gateway への接続は `Running` 到達後の購読プロセスが初めて行うため、認証要求は購読ループが `Error` へ落ちた時点の `IsAuthenticationRequired` に現れる
