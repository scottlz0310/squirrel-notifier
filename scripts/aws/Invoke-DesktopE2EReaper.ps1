...
.DESCRIPTION
  ...
  2. ephemeral-runner タグの volume のうち、どこにも接続されていない（available）ものを削除する。
     RunInstances の要求で DeleteOnTermination=false にされると terminate 後も残り、IAM では拒否できない
  3. terminate した instance と、破棄中・破棄済みの instance の JIT config parameter を削除する
     （parameter には有効期限ポリシーも付けるため、ここでの削除は二重の保険）
  4. 対応する instance が無くなった使い捨て runner の登録を削除する。候補は offline で job を実行して
...
# ephemeral-runner タグの volume がどこにも接続されていないのは、RunInstances の要求で
# DeleteOnTermination=false にされ、instance の terminate 後に残ったもの（IAM では拒否できない）。
$retainedVolumes = @(
    Invoke-AwsCli -Arguments @(
        'ec2', 'describe-volumes',
        '--region', $Region,
        '--filters', "Name=tag:$($tag.Key),Values=$($tag.EphemeralInstanceValue)", 'Name=status,Values=available',
...
{
    if ($PSCmdlet.ShouldProcess($volumeId, '残った使い捨て volume を削除'))
    {
